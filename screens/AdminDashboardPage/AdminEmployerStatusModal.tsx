...
type AdminEmployerStatusModalProps = {
...
const statusLabels: Record<EmployerInquiryStatus, string> = {
  cancelled: "Cancelled",
...
const statusDescriptions: Record<EmployerInquiryStatus, string> = {
  cancelled: "Stop this employer request and mark it as closed.",
  in_progress: "You are actively working on this employer request.",
  onboarded: "Employer onboarding is complete and ready to proceed.",
  pending: "New employer request waiting for admin review.",
};
...
const statusIcons = {
  ...
  pending: Clock3,
} satisfies Record<EmployerInquiryStatus, typeof Clock3>;

export const AdminEmployerStatusModal = ({
  employer,
  isOpen,
  onClose,
  onUpdated,
}: AdminEmployerStatusModalProps) => {
  const [selectedStatus, setSelectedStatus] = useState<EmployerInquiryStatus>("pending");
  ...
  useEffect(() => {
    setSelectedStatus(employer?.status ?? "pending");
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/admin/employers/${employer.id}/status`, {
      ...
      if (!response.ok) {
        setSaveState({
          message: data?.message ?? "Unable to update employer status right now.",
          type: "error",
        });
        return;
      }
      ...
      setSaveState({
        message: data?.message ?? "Employer status updated successfully.",
        type: "success",
      });
      onUpdated();
      onClose();
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          aria-label="Close employer status modal"
        ...
        <div className="bg-[linear-gradient(180deg,#eef9ff_0%,#ffffff_100%)] px-6 pb-7 pt-7 sm:px-8">
          ...
          <p className="mt-3 text-[15px] leading-[1.7] text-[#6b7894] sm:text-[16px]">
            Update the current workflow status for <strong>{employer.company}</strong>.
          </p>
        </div>
