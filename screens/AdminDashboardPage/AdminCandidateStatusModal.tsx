...
type AdminCandidateStatusModalProps = {
...
const statusDescriptions: Record<AdminCandidateAccountStatus, string> = {
  active:
    "Allow this candidate to sign in again and continue job applications normally.",
  suspended:
    "Suspend this candidate account to block access until an admin reactivates it.",
};

const statusIcons = {
  active: CheckCircle2,
  suspended: AlertTriangle,
} satisfies Record<AdminCandidateAccountStatus, typeof CheckCircle2>;

const statusLabels: Record<AdminCandidateAccountStatus, string> = {
  active: "Active",
  suspended: "Suspended",
};

const formatSuspendedUntil = (value: string | null) => {
  if (!value) {
    return null;
  }
...
export const AdminCandidateStatusModal = ({
  candidate,
  isOpen,
  onClose,
  onUpdated,
}: AdminCandidateStatusModalProps) => {
  const [selectedStatus, setSelectedStatus] =
    useState<AdminCandidateAccountStatus>("active");
  ...
  useEffect(() => {
    setSelectedStatus(candidate?.accountStatus ?? "active");
  ...
  const suspendedUntilLabel = formatSuspendedUntil(candidate.suspendedUntil);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/admin/candidates/${candidate.id}/status`, {
      ...
      if (!response.ok) {
        setSaveState({
          message:
            data?.message ?? "Unable to update candidate account status right now.",
          type: "error",
        });
        return;
      }
      ...
      setSaveState({
        message: data?.message ?? "Candidate account status updated successfully.",
        type: "success",
      });
      onUpdated();
      onClose();
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          aria-label="Close candidate status modal"
        ...
        <div className="bg-[linear-gradient(180deg,#eef9ff_0%,#ffffff_100%)] px-6 pb-7 pt-7 sm:px-8">
          ...
          <h2 className="mt-2 text-[32px] font-bold leading-[1.1] text-[#1d223f] sm:text-[38px]">
            Change candidate access
          </h2>
          <p className="mt-3 text-[15px] leading-[1.7] text-[#6b7894] sm:text-[16px]">
            Manage sign-in access for <strong>{candidate.candidateName}</strong>.
          </p>
          {candidate.accountStatus === "suspended" && suspendedUntilLabel ? (
            <p className="mt-3 text-[14px] font-medium text-[#b45309]">
              Current suspension is active until {suspendedUntilLabel}.
            </p>
          ) : null}
        </div>
