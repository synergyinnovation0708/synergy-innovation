...
const forgotPasswordHighlights = [
  {
    description: "Request a secure reset link without exposing the admin password.",
    icon: ShieldCheck,
    title: "Protected Recovery",
  },
  {
    description: "The reset journey routes back into the same Supabase admin account.",
    icon: BadgeCheck,
    title: "Same Admin Identity",
  },
  {
    description: "Choose a new password after the verified recovery link is opened.",
    icon: KeyRound,
...
export const AdminForgotPasswordPage = ({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setStatusMessage({
          tone: "error",
          value: data?.message ?? "Unable to send the admin reset email right now.",
        });
        return;
      }
      ...
      setStatusMessage({
        tone: "error",
        value: "Unable to send the admin reset email right now.",
      });
    } finally {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[linear-gradient(180deg,#edf7ff_0%,#ffffff_50%,#f7f8fc_100%)]">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1440px] items-center px-4 py-10 sm:px-6 lg:px-10">
        <div className="grid w-full gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          <section className="relative overflow-hidden rounded-[36px] bg-[#1d223f] px-6 py-8 text-white shadow-[0_32px_90px_rgba(29,34,63,0.24)] sm:px-10 sm:py-12">
            <div
              className="pointer-events-none absolute inset-0"
              aria-hidden
              style={{
            ...
            <div className="relative">
              ...
              <div className="mt-12 max-w-[560px]">
                ...
                <p className="mt-5 max-w-[520px] text-[18px] leading-[1.7] text-white/78 sm:text-[20px]">
                  Request a secure password reset link for the admin account and
                  continue back into the dashboard after verification.
                </p>
              </div>
              ...
              <div className="mt-10 rounded-[28px] border border-white/12 bg-white/6 p-6 backdrop-blur-sm">
                ...
                <p className="mt-2 max-w-[480px] text-[15px] leading-[1.7] text-white/72">
                  Use the same admin email that signs in to Supabase. After the
                  email link is verified, you can choose a fresh password.
                </p>
              </div>
            </div>
          </section>
            ...
            <div className="w-full rounded-[36px] border border-[#dce6f4] bg-white p-6 shadow-[0_30px_80px_rgba(29,34,63,0.12)] sm:p-8 lg:p-10">
              ...
              <div className="mt-6">
                ...
                <p className="mt-3 text-[16px] leading-[1.7] text-[#67728f]">
                  Enter the admin email and we&apos;ll send a reset link if the
              ...
              <form className="mt-10 space-y-6" onSubmit={handleSubmit}>
                <div>
                  ...
                  <input
                    ...
                    placeholder={emailHint || "admin@synergyinnovation.com"}
                    className={inputClassName}
                    disabled={isDisabled}
                    required
                  />
                  {fieldError ? (
                    <p className="mt-2 text-[13px] text-[#c05a5a]">{fieldError}</p>
                  ) : null}
                </div>
                ...
                <button
                  type="submit"
                  disabled={isDisabled}
                  className="group inline-flex h-[58px] w-full items-center justify-center gap-3 rounded-full bg-[#1d223f] px-6 text-[18px] font-semibold text-white transition-transform duration-200 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isSubmitting ? (
                    <>
                    ...
                    <>
                      ...
                      <span className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/70 bg-white/10">
              ...
              <div className="mt-8 rounded-[28px] bg-[#f5f9ff] p-5">
                <p className="text-[15px] font-semibold text-[#1d223f]">
                  Need to try signing in again?
                </p>
                <p className="mt-2 text-[14px] leading-[1.7] text-[#67728f]">
                  {isSupabaseConfigured
                    ? "Go back to the admin login page anytime if you remember the password."
                    : "Add the required Supabase environment variables before using admin password recovery."}
                </p>
                <Link
                  href="/admin/login"
