...
import { adminForgotPasswordPath } from "@/lib/admin-password-recovery";
...
const resetPasswordHighlights = [
  {
    description: "The password can only be changed after a verified recovery link is opened.",
    icon: ShieldCheck,
    title: "Verified Session",
  },
  {
    description: "Choose a new password that follows the platform password rules.",
    icon: KeyRound,
    title: "Secure Update",
  },
  {
    description: "After the reset, sign in again with your new credentials.",
    icon: BadgeCheck,
    title: "Back to Login",
  },
];
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setStatusMessage({
          tone: "error",
          value: data?.message ?? "Unable to reset the admin password right now.",
        });
        return;
      }
      ...
      setStatusMessage({
        ...
        value:
          data?.message ??
          "Admin password reset successfully. Sign in with your new password.",
      });
    } catch {
      setStatusMessage({
        tone: "error",
        value: "Unable to reset the admin password right now.",
      });
    } finally {
            ...
            <div className="relative">
              ...
              <div className="mt-12 max-w-[560px]">
                ...
                <p className="mt-5 max-w-[520px] text-[18px] leading-[1.7] text-white/78 sm:text-[20px]">
                  Finish the recovery flow by choosing a strong password for the
                  admin account, then sign in again with the updated credentials.
                </p>
              </div>
              ...
              {showRecoveryError ? (
                <div className="mt-8 rounded-[28px] bg-[#f5f9ff] p-5">
                  ...
                  <p className="mt-2 text-[14px] leading-[1.7] text-[#67728f]">
                    {isSupabaseConfigured
                      ? "This reset session is missing or has expired. Request a new admin reset email to continue."
                ...
                <div className="mt-8 rounded-[28px] bg-[#f5f9ff] p-5">
                  ...
                  <p className="mt-2 text-[14px] leading-[1.7] text-[#67728f]">
                    Your admin password has been reset. Return to the login page
                    ...
                    {isSubmitting ? (
                      ...
                      <>
                        ...
                        <span className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/70 bg-white/10">
                          <KeyRound className="h-4 w-4 transition-transform duration-200 group-hover:scale-105" />
                        </span>
                      </>
                    )}
                  </button>
